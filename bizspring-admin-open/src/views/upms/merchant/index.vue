  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="108px">
      <el-form-item label="用户名" prop="username">
        <el-input maxlength="30" v-model="queryParams.username"
          placeholder="请输入用户名" clearable style="width: 194px;"
    ...
    <el-table v-loading="loading" :data="merchantList" @selection-change="handleSelectionChange" :header-cell-style="{
      background:'#f8f8f9',height:'50px',color:'#606266'}" :rowStyle="{ height:'50px'}">
      <el-table-column label="用户名" align="center" prop="username" />
      <el-table-column label="邮箱" align="center" prop="email" />
      <el-table-column label="手机" align="center" prop="mobile" type />
    ...
    <el-dialog :title="title" v-model="open" width="550px" append-to-body :v-loading="loading">
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        <el-form-item label="用户名" prop="username">
          <el-input v-model="form.username" :disabled="isdisabled" maxlength="30"
            placeholder="请输入用户名" />
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="form.password" maxlength="20" show-password
            placeholder="请输入密码" />
        </el-form-item>
        <el-form-item label="确认密码" prop="passwordConfirm">
          <el-input v-model="form.passwordConfirm" maxlength="20" show-password placeholder="请输入确认密码" />
        </el-form-item>
        ...
        <el-form-item label="设置">
          <el-switch v-model="form.isEnabled" active-text="是否启用">
          </el-switch>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      ...
      uploadUrl: '/upms/file/upload?fileType=image&path=admin-upms-merchant',
  ...
  computed: {
    ...
    permissionList() {
      return {
        addBtn: this.permissions['store:merchant:add'] ? true : false,
        delBtn: this.permissions['store:merchant:del'] ? true : false,
        editBtn: this.permissions['store:merchant:edit'] ? true : false,
        viewBtn: this.permissions['store:merchant:get'] ? true : false,
      }
    },
  },
  methods: {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != null) {
            console.error('updateMerchant  form +++ > ', this.form)
            let data = {
              id: this.form.id,
    ...
    bindPickerChange: function (i, e) {
      this.idxs[i] = e.detail.value
    },

    change(index, e) {
      this.dataTimeValue[index] = e
    ...
    getList() {
      this.loading = true
      listMerchant(this.queryParams)
        .then((response) => {
          this.loading = false
          this.merchantList = response.data.data.content
    ...
    handleUpdate(row) {
      ...
      editMerchant(row.id)
        .then((response) => {
          console.log('editMerchant  ===   >>>', response)
          this.loading = false
          this.setRules()
